...
if(price_sale!=''){$(".price-sale").empty().text(price_regular)
$(".price-regular").empty().text(price_sale)}else{$(".price-sale").empty().text(price_regular)}
...
$('.header').append(mini);var modal=res.fragments['.modal-cart-content'];$(".modal-cart-add").empty().append(modal);$("#modalCart").modal('show');$("#myModal").modal('hide');},error:function(err){console.log(err);}})})}
...
$('.header').append(mini);$('.modal-cart-add').empty().append(modal);},error:function(err){console.log(err);}});return false;});$(document).on('click','.remove-product-variable',function(e){$(this).css('pointer-events','none')
...
$('.header').append(mini);var modal=data.fragments['.modal-cart-content'];$(".modal-cart-add").empty().append(modal);$(".total-price").text($('.total-price-dropdown').attr('data-total'))
...
$('.header').append(mini);var modal=res.fragments['.modal-cart-content'];$(".modal-cart-add").empty().append(modal);$('#cart-roll').animate({scrollTop:localStorage.scrollModal},800);}}});}
