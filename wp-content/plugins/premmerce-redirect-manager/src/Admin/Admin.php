...
class Admin
{
        ...
        if ($post->post_type == 'product') {
            ...
            if (in_array($post->post_status, array('draft', 'pending')) && $optionStatus && $update) {
                if ($post->post_name) {
                    ...
                    $this->model->deleteRedirect(array('old_url' => str_replace(get_site_url(), '', $url)));
                    ...
                    if ($categories) {
                        ...
                        $redirectType    = 'url';
                        $redirectContent = get_site_url();
                    }

                    $this->model->createRedirect(array(
                        'old_url'          => str_replace(get_site_url(), '', $url),
                ...
                $this->model->deleteRedirect(array('old_url' => str_replace(get_site_url(), '', get_permalink($pid))));
            }
        }
    }
    ...
    public function createRedirectOnTrashProduct($null, $post)
    {
        if ($post && $post->post_type == 'product') {
            if (!$this->model->getOneRedirectByOldUrl(str_replace(get_site_url(), '', get_permalink($post->ID)))) {
                if (!in_array($post->post_status, array('draft', 'pending'))) {
                    $oldUrl = str_replace(get_site_url(), '', get_permalink($post->ID));
    ...
    private function processingEdit()
    {
        ...
        $checkOldURL = $this->model->getOneRedirectByOldUrlAndOtherId($_POST['old_url'], $_POST['id']);
