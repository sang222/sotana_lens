...
class RedirectsTable extends \WP_List_Table
{
    ...
    {
        return '
            <a href="' . $_SERVER['REQUEST_URI'] . '&tab=edit&id=' . $item->id . '">' . $item->old_url . '</a>
            <div class="row-actions">
                ...
                <span class="delete">
                    <a data-link="delete" href="' . admin_url('admin-post.php') . '?action=premmerce_delete_redirect&id=' . $item->id . '">
...
//                ['redirect_type', 'NOT IN', ['product', 'product_category']]
    ...
    private function getContentUrl($contentType, $content)
    {
        switch ($contentType) {
            case 'url':
                $url = '<a href="' . $content . '" target="_blank">' . $content . '</a>';
                break;
